...
namespace Dal
{
    ...
    public class ExcelEdit
    {
        ...
        public void SetCellValue(string ws, string x, string y, object value)
        //ws：要设值的工作表的名称 X行Y列 value 值
        {
            Range range = GetSheet(ws).get_Range(x, y);
            range.Value = value;
        ...
        //ws：要获取的工作表的名称 X行Y列 value 值
        {
            Range range = ws.Cells[x, y];
            range.Select();
            return range.Value;
        }
        ...
        public void InsertTable(System.Data.DataTable dt, string ws, int startX, int startY, bool b)
        //将内存中数据表格插入到Microsoft.Office.Interop.Excel指定工作表的指定位置 为在使用模板时控制格式时使用一
        {
            ...
            if (b)
            {
                ...
                {
                    GetSheet(ws).Cells[1, j + startY] = dt.Columns[j].ToString();
